...
\begin{enumerate}
	...
	\item{
			...
			\scalebox{0.625}{
				\begin{circuitikz}
					...
					\draw (21,2) node[op amp] (opamp) {}
						...
						(opamp.out) to [short, -*] (22.5,2)
							to [short, -o] (23,2);
					\draw (23,0) to [open, v<=$V_o(s)$] (23,2);
				\end{circuitikz}
			}
		\end{figure}
		...
		\end{equation*}
		Now $V_n = 0$ V due to the virtual short-circuit condition at the op amp's input pins ($V_p - V_n = 0$ V) and $V_p = 0$ V. Therefore:
		\begin{equation}
			V_a = -I_a \cdot 2.47 \times 10^3
		\end{equation}
		Now find the current $I_{s1}$ by using Ohm's law:
		\begin{align*}
			I_{s1} &= \frac{V_i}{\, 2.47 \times 10^3 + \frac{\, 10^7 \,}{s} \,} \\
			&= \frac{\, s \cdot V_i /\left(2.47 \times 10^3\right) \,}{s + 4.04 \times 10^3}
		\end{align*}
		Now note that due to current constraint of the input pins of the op amp (no current flows into input pins), $I_{s1} = I_a$. Therefore, by equation (1):
		\begin{align*}
			V_a &= - \frac{\, s \cdot V_i /\left(2.47 \times 10^3\right) \,}{s + 4.04 \times 10^3} \cdot 2.47 \times 10^3 \\
